          ...
          <div className={`mx-auto transition-all duration-500 ease-in-out will-change-transform ${scrolled ? 'header-bg-scrolled' : 'header-bg'}`}
            ...
            <div className={`flex items-center justify-between transition-all duration-500 ease-in-out ${scrolled ? 'px-4 sm:px-6 h-14' : 'px-4 sm:px-6 h-16'} container mx-auto max-w-7xl`}>
              <Link href="/" className={`flex items-center gap-2 font-bold transform transition-all duration-500 flex-shrink-0 ${scrolled ? 'scale-90' : 'scale-100'} ${headerLoaded ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: headerLoaded ? '200ms' : '0ms' }}>
                <img src="/logo.webp" alt="BizAI Hacks Logo" width={scrolled ? 120 : 140} height={scrolled ? 30 : 35} className="transition-all duration-500 object-contain rounded-lg" style={{ width: 'auto', height: 'auto' }} />
              </Link>
...
=======
    <>
      <header className="sticky top-0 z-50 w-full">
        <div className={`relative transition-all duration-500 ease-in-out ${scrolled ? 'py-2 sm:py-3' : 'py-0'}`}>
          <div 
            className={`mx-auto transition-all duration-500 ease-in-out will-change-transform ${scrolled ? 'header-bg-scrolled' : 'header-bg'}`}
            style={{ 
          ...
          >
            <div className={`flex items-center justify-between transition-all duration-500 ease-in-out ${scrolled ? 'px-4 sm:px-6 h-14' : 'px-4 sm:px-6 h-16'} container mx-auto max-w-7xl`}>
              ...
              {/* ✅ FIX: Logo with aria-label */}
              <Link 
                href="/" 
                className={`flex items-center gap-2 font-bold transform transition-all duration-500 flex-shrink-0 ${scrolled ? 'scale-90' : 'scale-100'} ${headerLoaded ? 'opacity-100' : 'opacity-0'}`} 
                style={{ transitionDelay: headerLoaded ? '200ms' : '0ms' }}
              ...
              >
                <Image 
                  ...
                  height={scrolled ? 120 : 0} 
                  className="transition-all duration-500 object-contain rounded-lg" 
                  priority 
                />
              </Link>

              {/* ✅ FIX: Navigation with proper ARIA */}
              <nav 
                className={`hidden xl:flex items-center font-medium ${scrolled ? 'gap-1' : 'gap-2'}`}
                ...
                {navLinks.map((link, index) => (
                  ...
                  >
                    {link.hasDropdown ? (
                      <>
                        <button 
                          className={`nav-link flex items-center gap-1 px-3 min-h-[44px] ${scrolled ? 'text-sm' : 'text-base'} ${headerLoaded ? 'opacity-100' : 'opacity-0 scale-95'}`} 
                          style={{ transitionDelay: headerLoaded ? `${300 + index * 50}ms` : '0ms' }}
                        ...
                        <div 
                          ...
                          className={`nav-dropdown absolute top-full left-1/2 -translate-x-1/2 w-64 transition-all duration-300 origin-top ${activeDropdown === link.label ? 'opacity-100 scale-100 visible' : 'opacity-0 scale-95 invisible'}`}
                        >
                          <div className="py-2">
                            {link.dropdownItems?.map((item) => (
                              <Link 
                                ...
                                href={item.href} 
                                className="nav-dropdown-item min-h-[44px] flex items-center" 
...
=======
                      <Link 
                        href={link.href || "#"} 
                        className={`nav-link block px-3 min-h-[44px] flex items-center whitespace-nowrap ${scrolled ? 'text-sm' : 'text-base'} ${headerLoaded ? 'opacity-100' : 'opacity-0 scale-95'}`} 
                        style={{ transitionDelay: headerLoaded ? `${300 + index * 50}ms` : '0ms' }}
                      >
>>>>>>> 49348d4ed6f203e50201fa07449bba9a7f7d4c14
                        {link.label}
...
=======
              ...
              <div className={`hidden xl:flex items-center transform transition-all duration-500 flex-shrink-0 ${scrolled ? 'scale-90' : 'scale-100'} ${headerLoaded ? 'opacity-100' : 'opacity-0'}`} style={{ transitionDelay: headerLoaded ? '500ms' : '0ms' }}>
                <Button 
                  className={`btn-primary-custom whitespace-nowrap min-h-[44px] ${scrolled ? 'text-sm px-4 py-2' : 'text-base px-5 py-2.5'}`} 
                  asChild
                >
              ...
              {/* ✅ FIX: Mobile menu button with better ARIA */}
              <button 
                type="button" 
                onClick={toggleMenu} 
                className={`xl:hidden p-2 -mr-2 rounded-xl hover:bg-accent/80 focus:outline-none focus:ring-2 focus:ring-primary/50 transition-all z-50 relative min-h-[44px] min-w-[44px] flex items-center justify-center ${headerLoaded ? 'scale-100 opacity-100' : 'scale-75 opacity-0'}`} 
                style={{ transitionDelay: headerLoaded ? '400ms' : '0ms' }} 
...
=======
      {/* Mobile Menu Overlay */}
      <div 
        className={`xl:hidden fixed inset-0 bg-black/50 backdrop-blur-sm z-40 transition-opacity duration-300 ${isOpen ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} 
        onClick={() => setIsOpen(false)} 
        aria-hidden="true" 
      />
      
      {/* ✅ FIX: Mobile Menu with proper nav and role */}
      <nav
        id="mobile-menu"
        aria-label="Mobile navigation"
        className={`xl:hidden fixed left-4 right-4 top-24 z-50 transform transition-all duration-500 ease-in-out ${isOpen ? 'translate-y-0 opacity-100' : '-translate-y-10 opacity-0 pointer-events-none'}`}
      >
        <div className="mobile-menu-bg max-h-[calc(100vh-120px)] overflow-y-auto">
          <div className={`mobile-menu-top-bar transition-all duration-500 ease-out ${isOpen ? 'scale-x-100' : 'scale-x-0'}`} />
...
=======
                    <button 
                      ...
                      className="w-full relative py-3 px-4 nav-link text-left transition-all text-base font-medium min-h-[44px]"
                    ...
                    >
                      <div className="flex items-center justify-between">
                        ...
                        <ChevronDown 
                          className={`w-4 h-4 transition-transform duration-300 flex-shrink-0 ${mobileExpandedMenu === link.label ? 'rotate-180' : ''}`} 
                          aria-hidden="true"
                        />
                      </div>
                    </button>
                    <div 
                      ...
                      className={`overflow-hidden transition-all duration-500 ${mobileExpandedMenu === link.label ? 'max-h-96' : 'max-h-0'}`}
                    >
                      <div className="pl-6 pr-4 pb-2 space-y-1">
                        {link.dropdownItems?.map((item) => (
                          <Link 
                            key={`${item.href}-${item.label}`} 
                            href={item.href} 
                            className="block py-2.5 px-3 text-sm nav-link rounded-lg min-h-[44px] flex items-center" 
                            onClick={() => setIsOpen(false)}
                          >
>>>>>>> 49348d4ed6f203e50201fa07449bba9a7f7d4c14
                            {item.label}
...
=======
                  <Link 
                    href={link.href || "#"} 
                    className="block py-3 px-4 nav-link transition-all text-base font-medium min-h-[44px] flex items-center" 
                    onClick={() => setIsOpen(false)}
                  >
                ...
                <div className="mx-4 h-px bg-gradient-to-r from-transparent via-border/30 to-transparent" aria-hidden="true" />
              </div>
            ))}
            <div className="p-3 pt-4">
              <Button 
                className="w-full btn-primary-custom h-11 min-h-[44px] text-base font-semibold group" 
                asChild
              >
